...
Данные для работы: ваши расшифровки в *.eaf и *.textgrid.txt форматах. Файл в формате plain text для разметки [link](data/udpipe_example.txt).   
...
* Сделайте копию файла, которую нужно назвать `moscow.manual1.conllu` (где вместо `moscow` стоит ваша метка).  
